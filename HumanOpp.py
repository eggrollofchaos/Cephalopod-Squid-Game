...
class HumanOpp(BaseAI):
    ...
    def getMove(self, grid: Grid):
        ...
        while True:
            pos_input = input(
                "Where will you move to? Enter the coordinates (row #, column #): "
            )
            ...
            try:
                pos_input_list = (
                    pos_input.translate(TO_CLEAN).replace(" ", ",").strip().split(",")
                )  # sanitize
                x, y, *rest = tuple(
                    map(int, [num.strip() for num in pos_input_list if num])
                )  # sanitize more, take first 2 numbers
            except ValueError:
                (
                    cprint("Invalid entry, please try again.", "yellow")
                    if is_unix
            ...
            (
                cprint(f"You've entered ({x},{y}) as your move.", "yellow")
                if self.verbose and is_unix
                else None
            )
            ...
            try:
                ...
                (
                    cprint(
                        f"Invalid square: ({x},{y}) does not exist on this game board.",
                        "yellow",
                    )
                    if is_unix
                    else print(
            ...
            if (x, y) in available_moves:
                ...
                (
                    cprint("That move is invalid.", "yellow")
                    if is_unix
                    else print("That move is invalid.")
                )
                (
                    print(
                        f"Intended square ({x}, {y}) has value of {cell_value} and is a distance {grid_distance(self.pos,(x,y))} from current position."
                    )
                    if self.verbose
                    else None
                )
    ...
    def getTrap(self, grid: Grid):
        ...
        while True:
            ...
            try:
                trap_input_list = (
                    trap_input.translate(TO_CLEAN).replace(" ", ",").strip().split(",")
                )  # sanitize
                x, y, *rest = tuple(
                    map(int, [num.strip() for num in trap_input_list if num])
                )  # sanitize more, take first 2 numbers
            except ValueError:
                (
                    cprint("Invalid entry, please try again.", "yellow")
            ...
            (
                cprint(
                    f"You've entered ({x},{y}) as your intended trap position.", "red"
                )
                if self.verbose and is_unix
                else None
            )
            ...
            if (x, y) in available_cells:
                ...
                (
                    cprint("That trap position is invalid.", "yellow")
                    if is_unix
                    else print("That trap position is invalid.")
                )
                (
                    print(
                        f"Intended square ({x}, {y}) has value of {cell_value} and is a distance {grid_distance(opp_pos,(x,y))} from opponent's position."
                    )
                    if self.verbose
                    else None
                )
