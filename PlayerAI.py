...
class PlayerAI(BaseAI):
    ...
    def __init__(
        ...
        self.dim = (
            grid_size  # TODO: make a few heuristics dynamic based on this variable
        )
        ...
        self.max_search_traps = max(
            8, abs(self.max_search_traps)
        )  # no less than 8 to begin with
    ...
    def setPosition(
        self, new_position
    ):  # used by Game to set position once move is validated
    ...
    def getOpponentPosition(
        self, grid: Grid
    ):  # of current Grid object - CHECK when using?
    ...
    def getMove(self, grid: Grid) -> tuple:
        ...
        self.opp_pos = grid.find(
            3 - self.player_num
        )  # find opponent's current position using method
        ...
        if self.verbose:
            ...
            print(
                f"Player is at {self.pos}, Opponent is at {self.opp_pos}, they are currently {grid_distance(self.pos, self.opp_pos)} units apart. ",
                end="",
            )
            print(
                f"This is a heuristic of {PlayerAI.__near_opp_heur(grid, self.pos, self.opp_pos)}."
            )

            print(
                f"There are {PlayerAI.__avoid_traps_heur(grid, self.pos)[0]} trap(s) next to Player and {PlayerAI.__avoid_traps_heur(grid, self.opp_pos)[0]} trap(s) next to Opponent."
            )

        (
            print(f"Looking for best move. Depth limit is {self.depth_limit}.")
            if self.verbose
            else None
        )
        ...
        curr_conn_sq_heur_me, curr_conn_sq_me, curr_conn_sq_list_me = (
            self.__conn_sq_depth_lim_heur(
                grid, pos=self.pos, max_radius=3, only_heur=False
            )
        )
        curr_conn_sq_heur_opp, curr_conn_sq_opp, curr_conn_sq_list_opp = (
            self.__conn_sq_depth_lim_heur(
                grid, pos=self.opp_pos, max_radius=3, only_heur=False
            )
        )
        ...
        self.search_start_pos_me = self.__get_search_start_pos(
            grid, self.curr_conn_sq_list_opp, self.pos, self.opp_pos
        )  # get start square for Player trap search
        self.search_start_pos_opp = self.__get_search_start_pos(
            grid, self.curr_conn_sq_list_me, self.opp_pos, self.pos
        )  # get start square for Opponent trap search
        if self.verbose:
            if self.curr_conn_sq_me >= 28 or self.curr_conn_sq_opp >= 28:
                print(
                    f"Player's and opponent's component both have a lot of connected squares."
                )
            else:
                print(
                    f"Player's component has {int(self.curr_conn_sq_me)} connected squares, opponent's component has {int(self.curr_conn_sq_opp)}; max_radius=3."
                )
        ...
        match self.turns:
            ...
            case 6:
                ...
                self.graph_cut_size_cap = (
                    8  # starting lower for graph cut 2-ply, changed from 6
                )
                self.max_radius = (
                    3  # starting lower for graph cut 2-ply, changed from 2
                )
            case 7:
                # if self.turns == 7:
                # self.max_search_traps += 1
        ...
        alpha = -np.inf
        beta = np.inf
        max_move = self.__decision(
            grid, alpha, beta, player_num, opp_num, self.depth_limit
        )
        self.turns += 1
    ...
    def __get_valid_neighbors(grid: Grid, pos, radius=1) -> list:
        ...
        return [
            (a, b)
            for a in valid_range(x)
            for b in valid_range(y)
            if grid.map[(a, b)] == 0
        ]
    ...
    def __avoid_traps_heur(grid: Grid, pos) -> int:
        ...
        num_traps = len(
            [neighbor for neighbor in neighbors if grid.map[neighbor] == -1]
        )
    ...
    def __get_dof(self, grid: Grid, player_pos, opp_pos) -> tuple:
        ...
        center_heur = PlayerAI.__center_heur(grid, player_pos) - PlayerAI.__center_heur(
            grid, opp_pos
        )
        avoid_traps_heur = avoid_traps_heur_me - avoid_traps_heur_opp
        edge_touch_heur = PlayerAI.__edge_touch_heur(
            grid, player_pos
        ) - PlayerAI.__edge_touch_heur(grid, opp_pos)
        ...
        # else:
        n_conn_sq_heur_me, n_conn_sq_count_me, conn_sq_list_me = (
            self.__conn_sq_depth_lim_heur(
                grid, player_pos, max_radius=max_radius, only_heur=False
            )
        )
        n_conn_sq_heur_opp, n_conn_sq_count_opp, conn_sq_list_opp = (
            self.__conn_sq_depth_lim_heur(
                grid, opp_pos, max_radius=max_radius, only_heur=False
            )
        )
        ...
        if self.use_advanced_heuristics == "graphcut":
            ...
            turns_limit = (
                self.turns - 2 * np.ceil(self.depth_limit / 3) >= 0
            )  # starts at 4 if depth_limit in (4,5,6)
            # turns_limit = (self.turns - 2*np.ceil(self.depth_limit/3) >= 1) # starts at 5 if depth_limit in (4,5,6)
            conn_sq_limit_me = (
                self.curr_conn_sq_heur_me / 5 + 3 * (self.depth_limit // 4) <= 90
            )
            ...
            if (
                turns_limit and conn_sq_limit_me and conn_sq_list_me
            ):  # enabled graphcut if passes these conditions
                ...
                if self.use_graph_me == True:
                    graph_cut_heur_me = self.__graph_cut_heur(
                        grid,
                        player_pos,
                        comp_size=max_comp_size_me,
                        conn_sq_list=conn_sq_list_me[:max_comp_size_me],
                        max_radius=max_radius,
                    )

                if self.use_graph_opp == True:
                    graph_cut_heur_opp = self.__graph_cut_heur(
                        grid,
                        player_pos,
                        comp_size=max_comp_size_opp,
                        conn_sq_list=conn_sq_list_opp[:max_comp_size_opp],
                        max_radius=max_radius,
                    )

                if self.use_graph_d2_me == True:
                    graph_cut_heur_me = self.__graph_cut_2_ply_heur(
                        grid,
                        player_pos,
                        comp_size=max_comp_size_me,
                        conn_sq_list=conn_sq_list_me[:max_comp_size_me],
                ...
                if self.use_graph_d2_opp == True:
                    graph_cut_heur_opp = self.__graph_cut_2_ply_heur(
                        grid,
                        player_pos,
                        comp_size=max_comp_size_opp,
                        conn_sq_list=conn_sq_list_opp[:max_comp_size_opp],
    ...
    def __conn_sq_depth_lim_heur(
        self, grid: Grid, pos, max_radius=2, max_size=0, only_heur=True
    ) -> int:
        """
        ...
        search_frontier = PlayerAI.__get_valid_neighbors(
            grid, pos, radius=1
        )  # a list of positions
        conn_sq_list = [
            pos
        ]  # initialize connected squares list to include current player's square
        ...
        while search_frontier and radius <= max_radius:
            for this_pos in current_layer:
                if this_pos not in conn_sq_list:
                    ...
                    conn_sq_heur += 5 + 5 * (
                        max(3 - radius, 0) ** 2
                    )  # add to heuristic with weighting for closer squares
    ...
    def __graph_cut_heur(
        self, grid: Grid, pos, comp_size, conn_sq_list, max_radius=3
    ) -> int:
        """
        ...
        for trap_pos in conn_sq_list:  # iterate over connected squares
            ...
            _, new_comp_size, _ = self.__conn_sq_depth_lim_heur(
                grid_clone,
                pos,
                max_radius=max_radius,
                max_size=comp_size,
            ...
            utility = (
                5 * size_delta - (7 - new_comp_size) ** 2 - 0.1 * random.random()
            )  # using a randomness to help with tie breaking
    ...
    def __graph_cut_2_ply_heur(
        ...
        def __gch_d2(grid: Grid, trap_pos, comp_size) -> int:
            ...
            _, new_comp_size, _ = self.__conn_sq_depth_lim_heur(
                grid, pos, max_radius=max_radius, max_size=comp_size, only_heur=False
            )
            grid.map[trap_pos] = 0
            size_delta = new_comp_size - comp_size
            ...
            utility = (
                5 * size_delta - (7 - new_comp_size) ** 2 - 0.1 * random.random()
            )  # updating, removed the j (from enumeration)
        ...
        for trap_pos in conn_sq_list:  # iterate over connected squares
            ...
            for trap_pos_2 in conn_sq_list_2:
                ...
                utility = (
                    __gch_d2(grid_clone, trap_pos_2, comp_size) - 0.1 * random.random()
                )  # updating, removed the i (from enumeration)
                # graph_cut_heur += utility
                graph_cut_heur.append(utility)
            grid_clone.map[trap_pos] = 0
    ...
    def __get_heuristics(self, grid: Grid, player_num, opp_num) -> tuple:
        ...
        if self.use_advanced_heuristics == "graphcut":
            ...
            return grid, PlayerAI.__n_neighbors_heur(
                grid, player_pos
            ) - PlayerAI.__n_neighbors_heur(grid, opp_pos)
            # if self.verbose:
            #     start = time.time()
    ...
    def __get_search_start_pos(
        self, grid: Grid, curr_conn_sq_list_target: list, attack_pos, target_pos
    ) -> tuple:
        """
        ...
        while (
            start_pos not in curr_conn_sq_list_target
        ):  # taking dynamically from argument
            if self.verbose > 2:
            ...
            start_pos = PlayerAI.__find_center(start_pos, target_pos)
            (
                print(f"New trap search start candidate: {start_pos}.")
                if self.verbose > 2
                else None
            )
    ...
    def __get_trap_candidates(
        self, grid: Grid, player_pos, opp_pos, turn="player"
    ) -> list:
        """
        ...
        if turn == "opp":  # i.e. Player is the target
            _, _, curr_conn_sq_list = self.__conn_sq_depth_lim_heur(
                grid, pos=player_pos, max_radius=3, only_heur=False
            )
            attack_pos = opp_pos
            ...
            _, _, curr_conn_sq_list = self.__conn_sq_depth_lim_heur(
                grid, pos=opp_pos, max_radius=3, only_heur=False
            )
            attack_pos = player_pos
            target_pos = opp_pos
            # start_pos = self.__get_search_start_pos(grid, self.curr_conn_sq_list_opp, player_pos, opp_pos)

        start_pos = self.__get_search_start_pos(
            grid, curr_conn_sq_list, attack_pos, target_pos
        )  # making this dynamic too
        ...
        while search_frontier and radius <= max_radius:
            ...
            if (
                pos in curr_conn_sq_list
                and pos not in trap_candidates
                and not grid.map[pos]
            ):
                ...
                trap_count += 1
            if (
                trap_count >= self.max_search_traps
            ):  # finish once we've collected the max # of candidates
            ...
            while not new_neighbors:
                if (
                    self.turns <= 3
                ):  # strategy optimization, don't consider trap positions on a edge until turn 4
                    # print("Don't consider traps on edges.") if self.verbose > 0 else None
                    new_neighbors = PlayerAI.__get_valid_neighbors_avoid_edge(
                        grid, start_pos, radius
                    )
                if not new_neighbors:
                    (
                        print(
                            f"Somehow searching traps on edges despite being at turn {self.turns}."
                        )
                        if self.verbose > 0 and self.turns <= 3
                        else None
                    )
                    new_neighbors = PlayerAI.__get_valid_neighbors(
                        grid, start_pos, radius
                    )
                radius += 1
        ...
        if trap_count == 0:
            if self.verbose > 1:
                print(
                    f"No trap positions left after visiting {self.child_nodes_seen} child nodes."
                )
            catch_trap = PlayerAI.__get_valid_neighbors_avoid_edge(
                grid, start_pos, radius
            )
            if not catch_trap:
    ...
    def __trap_minimize(
        self, grid: Grid, alpha, beta, player_num, opp_num, depth, depth_limit
    ) -> tuple:
        """
        ...
        player_pos = tuple(np.argwhere(grid.map == player_num)[0])
        opp_pos = tuple(
            np.argwhere(grid.map == opp_num)[0]
        )  # this is the position in question
        gameover_result = PlayerAI.__is_over(
            grid, player_num, player_pos, opp_num, opp_pos
        )
        if gameover_result:
        ...
        cache = {}
        player_pos_a = np.array(
            player_pos
        )  # using numpy arrays to allow for faster arithmetic
        opp_pos_a = np.array(opp_pos)
        x_0, x_1 = player_pos_a - opp_pos_a
        center_a = np.array(PlayerAI.__find_center(player_pos, opp_pos))

        for trap_pos in self.__get_trap_candidates(
            grid, player_pos, opp_pos, turn="opp"
        ):
            ...
            if key in cache:
                ...
                _, utility = self.__move_maximize(
                    grid, alpha, beta, player_num, opp_num, depth + 1, depth_limit
                )
                ...
                dist_mid = abs(player_pos_a - opp_pos_a) - 2 * abs(
                    center_a - trap_pos_a
                )  # heuristic tested on paper
                if x_0 > x_1:
            ...
            for neighbor in neighbors:
                if grid.map[neighbor] in (
                    0,
                    -1,
                ):  # to avoid player and opponent location, but can land on an existing trap
            ...
            if expected_utility < minUtility:
                ...
                if self.verbose > 2:
                    if minUtility < self.trap_min_utility:
                        ...
                        cprint(
                            f"Found better (lower) trap utility of {minUtility}.",
                            color="red",
                        )
                        pass
            ...
            if utility < minUtility:
                ...
                if self.verbose > 2:
                    if minUtility < self.move_min_utility:
                        ...
                        cprint(
                            f"Found better (lower) move utility of {minUtility}.",
                            color="yellow",
                        )
                        pass
    ...
    def __trap_maximize(
        ...
        player_pos = tuple(
            np.argwhere(grid.map == player_num)[0]
        )  # this is the position in question
        opp_pos = tuple(np.argwhere(grid.map == opp_num)[0])
        gameover_result = PlayerAI.__is_over(
            grid, player_num, player_pos, opp_num, opp_pos
        ...
        for trap_pos in self.__get_trap_candidates(
            grid, player_pos, opp_pos, turn="player"
        ):
            # initialize with the main trap's probability-weighted utility, then move on to those of the neighbors
            # this is because each throw has a small chance of landing on a square adjacent to intended square
            if self.verbose > 1:
            ...
            if expected_utility > maxUtility:
                ...
                if self.verbose > 2:
                    if maxUtility > self.trap_max_utility:
                        ...
                        cprint(
                            f"Found better (high) trap utility of {maxUtility}.",
                            color="green",
                        )
                        pass
            ...
            if maxUtility > alpha:
                ...
                if self.verbose > 2:
                    if maxUtility > self.move_max_utility:
                        ...
                        cprint(
                            f"Found better (high) move utility of {maxUtility}.",
                            color="cyan",
                        )
                        pass

        self.optimal_trap_pos = (
            maxTrap  # maxTrap is now just a position, not a Grid object
        )
    ...
    def __decision(
        ...
        if self.verbose:
            start = time.time()
            child, self.utility = self.__move_maximize(
                grid, alpha, beta, player_num, opp_num, depth=0, depth_limit=depth_limit
            )
            print(
                f"Max trap candidates seen from one child: {self.max_trap_candidates}"
            )
            if self.use_advanced_heuristics:
                ...
                if self.use_graph_me:
                    cprint(
                        f"Used graph cut heuristic on player, size_cap={self.graph_cut_size_cap}, max_radius={self.max_radius}.",
                        color="blue",
                    )
                if self.use_graph_d2_me:
                    cprint(
                        f"Used 2-ply graph cut heuristic on player, size_cap={self.graph_cut_size_cap}, max_radius={self.max_radius}.",
                        color="blue",
                    )
                if self.use_graph_opp:
                    cprint(
                        f"Used graph cut heuristic on opponent, size_cap={self.graph_cut_size_cap}, max_radius={self.max_radius}.",
                        color="red",
                    )
                if self.use_graph_d2_opp:
                    cprint(
                        f"Used 2-ply graph cut heuristic on opponent, size_cap={self.graph_cut_size_cap}, max_radius={self.max_radius}.",
                        color="red",
                    )
            ...
            if self.utility >= 90000:
                ...
                (
                    ...
                    if is_unix
                    else print("Not looking so good :/")
                )
                ...
                (
                    ...
                    if is_unix
                    else print("Hey, doing what we can :|")
                )
            end = time.time()
    ...
    def getTrap(self, grid: Grid) -> tuple:
        ...
        if self.optimal_trap_pos == self.current_move:
            ...
            (
                input(
                    "We've taken up the last spot. Throwing trap randomly because we'll win anyway."
                )
                if self.verbose == 3
                else None
            )
        ...
        (
            print(
                f"Using cached optimal trap position that was found during Expectiminimax."
            )
            if self.verbose
            else None
        )
