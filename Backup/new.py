...
class PlayerAI(BaseAI):
    ...
    def getMove(self, grid: Grid) -> tuple:
        ...
        current_conn_sq = (
            self.__connected_sq_heur(grid, pos=self.getPosition(), is_me=True) / 5
        )
    ...
    def __get_dof(self, grid: Grid, is_me) -> tuple:
        ...
        if self.turns >= 2:
            n_neighbors_heur = self.__n_neighbors_heur(
                grid, is_me=True
        ...
        if self.turns >= 4:
            edge_touch_heur = self.__edge_touch_heur(
                grid, is_me=True
            ) - self.__edge_touch_heur(grid, is_me=False)
        if self.turns >= 5:
            n_conn_sq_heur_me, conn_sq_list = self.__connected_sq_heur(
                grid, return_pos=True, is_me=True
            )
        if self.turns >= 6:
            n_conn_sq_heur_me, conn_sq_list_me = self.__connected_sq_heur(
                grid, return_pos=True, is_me=True
            )
            n_conn_sq_heur_opp, conn_sq_list_opp = self.__connected_sq_heur(
                grid, return_pos=True, is_me=False
            )
            # grid.print_grid()
        ...
        if self.turns >= 12 or self.current_conn_sq < 24:
            ...
            graph_cut_heur = self.__graph_cut_heur(
                grid, comp_size=n_conn_sq_heur_me, conn_sq=conn_sq_list_me, is_me=True
            )
        ...
        if self.turns >= 12 or self.current_conn_sq < 21:
            ...
            graph_cut_heur = self.__graph_cut_heur(
                grid,
                comp_size=n_conn_sq_heur_opp,
                conn_sq=conn_sq_list_opp,
                is_me=False,
            )
        ...
        return (
            grid,
            n_conn_sq_heur + n_neighbors_heur + edge_touch_heur + graph_cut_heur,
        )
    ...
    def __connected_sq_heur(
        self, grid: Grid, return_pos=False, pos=None, is_me=True
    ) -> tuple:
        """
    ...
    def __graph_cut_heur(self, grid: Grid, comp_size, conn_sq, is_me=True) -> int:
        ...
        if is_me:
            ...
            pos = self.getOpponentPosition(grid)
        conn_comps = self.__num_connected_components(
            grid, is_me=is_me
        ...
        for trap_pos in conn_sq:  # iterate over connected squares
            ...
            new_comp_size = self.__connected_sq_heur(
                grid, is_me=is_me, return_pos=False
            )
    ...
    def __num_connected_components(
        self, grid: Grid, return_labels=False, is_me=True
    ) -> int:
        """
        ...
        return connected_components(
            csgraph=graph, directed=False, return_labels=return_labels
        )
    ...
    def __get_trap_candidates(self, grid: Grid, pos) -> list:
        ...
        while radius <= threshold:
            ...
            for pos in search_frontier:  # iterate over all search frontier
                if (
                    pos not in explored and grid.getCellValue(pos) == 0
        ...
        for trap_pos in self.__get_trap_candidates(grid, player_position):
            ...
            if key in cache:
                ...
                _, utility = self.__move_maximize(
                    grid, alpha, beta, depth + 1, depth_limit
                )
                cache[key] = utility
    ...
    def __move_minimize(self, grid: Grid, alpha, beta, depth, depth_limit) -> tuple:
        ...
        for neighbor_to_move in grid.get_neighbors(
            opponent_position, only_available=True
        ):
    ...
    def __move_maximize(self, grid: Grid, alpha, beta, depth, depth_limit) -> tuple:
        ...
        for neighbor_to_move in grid.get_neighbors(
            player_position, only_available=True
        ):
    ...
    def __tie_break(
        self, Move_A: Grid, Move_B: Grid, Trap_A: Grid, Trap_B: Grid, is_me
    ) -> tuple:
        """
        ...
        num_traps = len(
            [neighbor for neighbor in neighbors if Move_A.map[neighbor] == -1]
        )
        marginal_utility_A -= num_traps
        neighbors = Move_B.get_neighbors(pos_B)
        num_traps = len(
            [neighbor for neighbor in neighbors if Move_B.map[neighbor] == -1]
        )
