...
def main():
    ...
    clear = lambda: system(
        "clear"
    )  # pretty cool way to clear the output, should look into more
    ...
    opp_depth_limit_def = (
        False  # flag for if we need to set the default depth limit for AI using Minimax
    )
    ...
    if len(argv) > 1:
        ...
        if opp_ai_int > 0 and "-od" in argv:
            ...
            "-od" in argv
        ):  # for Easy AI, Medium AI, or Human Opponent but depth limit argument passed
        ...
        if (
            "-g" in argv
        ):  # this means to show stdout and stderr in terminal, instead of capturing
    ...
    if is_unix:  # if Unix, print in color
        ...
        cprint(
            " " * 11 + "AI SQUID GAME - BATCH TEST SCRIPT" + " " * 11,
            color="blue",
            on_color="on_yellow",
            attrs=["bold"],
        )
        ...
        cprint(
            f"Setting Player AI search depth limit to {depth_limit}.", "green"
        )  # if depth_limit:
        if heur:
        ...
        if (
            opp_ai_int > 0 and opp_ai_int != 9 and opp_depth_limit
        ):  # if above Easy/Medium AI and not Human Opponent
            if opp_depth_limit_def:
                cprint(
                    "Opponent AI search depth limit not specified, defaulting to ",
                    "magenta",
                    end="",
                )
            else:
    ...
    total_time, run_times, rounds_list, run_success, player_wins = (
        run_games.start_batch()
    )
    ...
    with open(results_filename, "a") as f:
        ...
        if run_success != 0:
            f.write(
                f"Of those, Player won {player_wins} times, for a win rate of {win_rate:.2f}%.\n"
            )  # , 'green')
            error_str = "complete"
        else:
        ...
        f.write(
            f"Each run took an average of {total_time/n:.2f} seconds to {error_str}.\n"
        )
        f.write(
            f"Max runtime: {max(run_times) :.2f} seconds; min runtime: {min(run_times):.2f} seconds.\n"
        )
        f.write(f"The average game length is {avg_rounds:.1f} rounds.\n")
        f.write(
            f"Max game length: {max(rounds_list)} rounds; min game length: {min(rounds_list)} rounds.\n"
        )
        (
