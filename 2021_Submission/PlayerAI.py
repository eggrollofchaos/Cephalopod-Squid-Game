...
class PlayerAI(BaseAI):
    def __init__(
        self, depth_limit=DEFAULT_DEPTH_LIMIT, heur="graphcut", verbose=False
    ) -> None:
    ...
    def getMove(self, grid: Grid) -> tuple:
        ...
        )  # find opponent's current position
        ...
        curr_conn_sq_me, curr_conn_sq_list_me = self.__conn_sq_depth_lim_heur(
            grid, pos=self.pos, max_radius=3, return_pos=True
        )
        curr_conn_sq_opp, curr_conn_sq_list_opp = self.__conn_sq_depth_lim_heur(
            grid, pos=self.opp_pos, max_radius=3, return_pos=True
        )
        ...
        self.search_start_pos = self.__get_search_start_pos(
            grid, self.pos, self.opp_pos
        )  # get square to start trap searches on
        if self.verbose:
        ...
        self.max_search_traps += min(
            2 * (5 - self.depth_limit), 1
        )  # adjust based on depth_level
        if self.turns == 2:
    ...
    def __get_dof(self, grid: Grid, player_pos, opp_pos) -> tuple:
        ...
        if self.turns <= 3:
            center_heur = self.__center_heur(grid, player_pos) - self.__center_heur(
                grid, opp_pos
            )
            near_opp_heur = self.__near_opp_heur(grid, player_pos, opp_pos)
            edge_touch_heur = self.__edge_touch_heur(
                grid, player_pos
            ) - self.__edge_touch_heur(grid, opp_pos)
            avoid_traps_heur = self.__avoid_traps_heur(
                grid, player_pos
            ) - self.__avoid_traps_heur(grid, opp_pos)
        ...
        if self.turns <= 3:
            conn_sq_depth_lim_me, conn_sq_list_me = self.__conn_sq_depth_lim_heur(
                grid, player_pos, max_radius=3, return_pos=True
            )
            conn_sq_depth_lim_opp, conn_sq_list_opp = self.__conn_sq_depth_lim_heur(
                grid, opp_pos, max_radius=3, return_pos=True
            )
        else:
            ...
            n_conn_sq_heur_me, conn_sq_list_me = self.__conn_sq_depth_lim_heur(
                grid, player_pos, max_radius=4, return_pos=True
            )
            n_conn_sq_heur_opp, conn_sq_list_opp = self.__conn_sq_depth_lim_heur(
                grid, opp_pos, max_radius=4, return_pos=True
            )
    ...
    def __conn_sq_depth_lim_heur(
        self, grid: Grid, pos, max_radius=2, max_size=0, return_pos=False
    ) -> int:
        """
    ...
    def __graph_cut_heur(
        ...
        for i, trap_pos in enumerate(conn_sq_list):  # iterate over connected squares
            ...
            new_comp_size = self.__conn_sq_depth_lim_heur(
                grid_clone,
                pos,
                max_radius=max_radius,
                max_size=comp_size,
    ...
    def __graph_cut_2_ply_heur(
        ...
        def __gch_d2(grid: Grid, trap_pos, comp_size) -> int:
            ...
            new_comp_size = self.__conn_sq_depth_lim_heur(
                grid, pos, max_radius=max_radius, max_size=comp_size, return_pos=False
            )
            grid.map[trap_pos] = 0
            size_delta = new_comp_size - comp_size
    ...
    def __get_heuristics(self, grid: Grid, player_num, opp_num) -> tuple:
        ...
        if self.use_advanced_heuristics == "graphcut":
            ...
            return grid, self.__n_neighbors_heur(
                grid, player_pos
            ) - self.__n_neighbors_heur(grid, opp_pos)
    ...
    def __get_trap_candidates(self, grid: Grid, pos) -> list:
        ...
        """
        start_pos = (
            self.search_start_pos
        )  # find the ideal spot to start trap candidate search
        ...
        while search_frontier and radius <= max_radius:
            ...
            if (
                pos in self.curr_conn_sq_list_opp
            ...
            while not new_neighbors:
                if self.turns <= 4:  # strategy optimization
                    new_neighbors = self.__get_valid_neighbors_avoid_edge(
        ...
        for trap_pos in self.__get_trap_candidates(grid, player_pos):
            ...
            for neighbor in neighbors:
                if grid.map[neighbor] in (
                    0,
                    -1,
                ):  # to avoid player and opponent location
    ...
    def __decision(
        ...
        if self.verbose:
            ...
            if self.utility >= 90000:
                print(
                    f"Best move found has utility of {self.utility:.2f}. Win imminent? 😱"
                )
            elif self.utility >= 1000:
                print(
                    f"Best move found has utility of {self.utility:.2f}. Doing decent! 😲"
                )
            elif self.utility <= -1000:
                print(
                    f"Best move found has utility of {self.utility:.2f}. Not looking so good. 😟"
                )
            elif self.utility <= -90000:
                print(
                    f"Best move found has utility of {self.utility:.2f}, omg no... 😵‍💫"
                )
            else:
                print(
                    f"Best move found has utility of {self.utility:.2f}. Hey, doing what we can. 😐"
                )
            # end = time.time()
