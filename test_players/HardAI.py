...
class HardAI(BaseAI):
    ...
    def maximize_move(self, grid: Grid, alpha: float, beta: float, depth: int, start_time: float) -> (tuple[int, int], float):
        """ 
        Description
        -----------
    ...
    def maximize_trap(self, grid: Grid, alpha: float, beta: float, depth: int, start_time: float) -> (tuple[int, int], int):
