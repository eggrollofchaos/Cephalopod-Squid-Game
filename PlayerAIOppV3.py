...
class PlayerAIOppV3(BaseAI):
    ...
    def __init__(
        ...
        self.max_search_traps = min(
            5, abs(self.max_search_traps)
        )  # no less than 5 to begin with
    ...
    def getMove(self, grid: Grid) -> tuple:
        ...
        match self.turns:
            ...
            case 6:
                ...
                self.max_radius = (
                    2  # starting lower for graph cut 2-ply, changed from 2
                )
            case 7:
                # if self.turns == 7:
                # self.max_search_traps += 1
    ...
    def __get_dof(self, grid: Grid, player_pos, opp_pos) -> tuple:
        ...
        if self.turns <= 3:
            conn_sq_depth_lim_me, conn_sq_list_me = self.__conn_sq_depth_lim_heur(
                grid, player_pos, max_radius=2, return_pos=True
            )
            conn_sq_depth_lim_opp, conn_sq_list_opp = self.__conn_sq_depth_lim_heur(
                grid, opp_pos, max_radius=2, return_pos=True
            )
        else:
