...
class PlayerAIOppV2(BaseAI):
    ...
    def getMove(self, grid: Grid) -> tuple:
        ...
        self.reached_max_search = False
        self.opp_pos = self.getOpponentPosition(
            grid
        ...
        curr_conn_sq_me, curr_conn_sq_list_me = self.__connected_sq_heur(
            grid, pos=self.pos, max_size=27, return_pos=True
        )
        curr_conn_sq_opp, curr_conn_sq_list_opp = self.__connected_sq_heur(
            grid, pos=self.opp_pos, max_size=27, return_pos=True
        )
        ...
        turn_adjust = int(
            ((self.turns + 5) ** 2) / 30 - (1 / 2) * (self.turns + 5)
        )  # adjustment based on turn
    ...
    def __is_over(self, grid: Grid, turn) -> int:
        ...
        opponent_neighbors = self.__get_valid_neighbors(
            grid, self.getOpponentPosition(grid)
        )
        if len(opponent_neighbors) == 0:
            return self.getPlayerNum()

        # check if Opponent has won
        player_neighbors = self.__get_valid_neighbors(
            grid, self.getPlayerPosition(grid)
        )
    ...
    def __connected_sq_heur(
        self, grid: Grid, pos, max_size=27, return_pos=False
    ) -> tuple:
        """
    ...
    def __get_heuristics(self, grid: Grid) -> tuple:
        ...
        return grid, self.__n_neighbors_heur(grid, pos) - self.__n_neighbors_heur(
            grid, opp_pos
        )
    ...
    def __get_trap_candidates(self, grid: Grid, pos) -> list:
        ...
        while search_frontier and radius <= max_radius:
            ...
            if self.verbose and trap_count >= self.max_search_traps:
                ...
                print(
                    f"Reached max # of trap positions to consider after visiting {self.child_nodes_seen} child nodes."
                )
